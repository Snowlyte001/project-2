    ...
    try {
      ...
      if (selectedProvider) {
        ...
        try {
          ...
          finalResponse = this.generateEnhancedOfflineResponse(question, webSearchResults, context);
          confidence = 0.6;
        }
        ...
        finalResponse = this.generateEnhancedOfflineResponse(question, '', context);
        confidence = 0.6;
      }
...
Always be empathetic and supportive while staying focused and helpful.`;
  ...
  private static generateEnhancedOfflineResponse(question: string, webSearchResults?: string, context?: AIContext): string {
  ...
  private static handleGreeting(message: string): AIResponse {
    const responses = [
